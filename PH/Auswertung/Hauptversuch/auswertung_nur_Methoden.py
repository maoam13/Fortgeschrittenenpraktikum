...
def nan_aus_array(array):
    #Entfernt nan aus Arrays
    #ACHTUNG: Es dürfen keine "0" im Array stehen, diese werden auch entfernt!!!
    out = []
...
def nan_aus_daten(x, y):
    #Entfernt Datenpunkte, bei denen der y-Wert nan ist
    #ACHTUNG: Es dürfen keine "0" im Array stehen, diese werden auch entfernt!!!
    x_out = []
    y_out = []
    y = np.nan_to_num(y)
...
def get_werte_ab_schwelle(array, schwelle):
    index = []
    for i in range(len(array)):
        if array[i] > schwelle:
            index.append(i)
...
def chiq(x, y, ex, ey, func, abl, A):
    chi = 0
    for i in range(len(x)):
        chi += (y[i] - func(A, x[i]))**2/((ex * abl(A, x[i]))**2 + ey**2)
        #print chi
    return chi/(len(x) - 1)

def chiq_y(x, y, ey,func, A):
    chi = 0
    for i in range(len(x)):
        chi += (y[i] - func(A, x[i]))**2/(ey[i]**2)
    return chi/(len(x) - 1)

def sortieren(B, i):
    A = np.copy(B)
    if i >= len(A):
        print("Index ", i, " is out of bounds for data with range ", len(A))
        return 0
    rueck = []
    for j in range(len(A)):
        rueck.append(np.full(len(A[j]), 0.1))
    for j in range(len(A[i])):
        minn, k = minausarray(A[i])
        for n in range(len(A)):
            rueck[n][j] = A[n][k]
        A[i][k] = 1.2 * max(A[i])
    return rueck

def untermenge_daten(x,y,ex,ey,x0,x1):
    '''
    ...
    return (np.array(xn),np.array(yn),np.array(exn),np.array(eyn))