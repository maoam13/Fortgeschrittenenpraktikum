...
plt.errorbar(T_CW, chi_CW, xerr = sT_CW, yerr = schi_CW, fmt = '.', color = 'b', label = 'Messwerte')
plt.figtext(0.15,0.65,
            'Model: y = a $\cdot$ x + b \n'
            +'$a$= ('+ str(np.round(sol_CW[0],5)) + ' $\pm$ '+ str(np.round(sol_CW[1],5)) + ') 1/K \n'
            +'$b$= '+ str(np.round(sol_CW[2],5)) + ' $\pm$ '+ str(np.round(sol_CW[3],5)) + ' \n'
...
plt.errorbar(T_CW, H, yerr = H_err, fmt = '.', color = 'b')
...
plt.errorbar(T_g, chi_g, xerr = sT_g, yerr = schi_g, fmt = '.', color = 'b', label = 'Messwerte')
plt.figtext(0.15,0.65,
            'Model: y = a $\cdot$ x + b \n'
            +'$a$= ('+ str(np.round(sol_g[0],5)) + ' $\pm$ '+ str(np.round(sol_g[1],5)) + ') 1/K \n'
            +'$b$= '+ str(np.round(sol_g[2],5)) + ' $\pm$ '+ str(np.round(sol_g[3],5)) + ' \n'
...
plt.errorbar(T_g, H, yerr = H_err, fmt = '.', color = 'b')
plt.axhline(0.0, color = 'r')
plt.ylabel('Residuen')
plt.xlabel('T [K]')
