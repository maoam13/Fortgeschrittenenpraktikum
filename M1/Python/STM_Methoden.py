...
#AFM
def multi_lin_reg_one_plot(x, y, sig_x, sig_y, start_index, end_index, labels, save_path):
    color = ['r', 'y', 'g', 'm', 'c', 'k']
    ...
    for i in range(len(start_index)):
        H = np.zeros(end_index[i] - start_index[i])
        H_err = np.zeros(end_index[i] - start_index[i])
        for j in range(len(H)):
            H[j] = y[start_index[i] + j] - lin(x[start_index[i] + j], out[i][0], out[i][2])
            H_err[j] = np.sqrt(sig_y[start_index[i] + j]**2 + (out[i][0] * sig_x[start_index[i] + j])**2) 
