...
for i in range(1, 7):
    ...
    G_step.append(__G[2])
    D_step.append(__D[2])
    sig_G_step.append(__G[3])
    sig_D_step.append(__D[3])
...
for i in range(1, 4):
    ...
    __G, __D = pf.peaks(name)
    G_wrinkle.append(__G[2])
    D_wrinkle.append(__D[2])
    sig_G_wrinkle.append(__G[3])
    sig_D_wrinkle.append(__D[3])
...
for i in range(1, 6):
    ...
    G_PMMA.append(__G[2])
    D_PMMA.append(__D[2])
    sig_G_PMMA.append(__G[3])
    sig_D_PMMA.append(__D[3])
...
plt.xlabel('$\Gamma _G$ [cm$^{-1}$]', **font)
plt.ylabel('$\Gamma _{2D}$ [cm$^{-1}$]', **font)
plt.legend()
plt.grid()
plt.tight_layout()
#plt.savefig('test.png')
plt.savefig('../Protokoll/Bilder/Part_3/gamma_2D_vs_G.png')
plt.close(fig)
